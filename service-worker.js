...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(fetch(event.request).then(async response => {
    if(url.pathname.endsWith('/transfers.html') || url.pathname.endsWith('/transfers.html/')){
      try{const text=await response.clone().text();if(!text.includes('afrn-transfer-workflow.js')){const injected=text.replace(/<\/body>/i,'<script src="./afrn-transfer-workflow.js"></script></body>');response=new Response(injected,{status:response.status,statusText:response.statusText,headers:response.headers});}}catch(e){}
    }
