const CACHE_NAME = 'afrn-football-v4';
const APP_SHELL = [
  ...
  './supabase-config.js', './afrn-transfer-workflow.js', './manifest.json', './css/style.css',
  ...
  event.respondWith(fetch(event.request).then(async response=>{
    if(url.pathname.endsWith('/transfers.html') || url.pathname.endsWith('/transfers.html/')){
      try{
        ...
        if(text.includes('afrn-transfer-workflow.js')===false){
