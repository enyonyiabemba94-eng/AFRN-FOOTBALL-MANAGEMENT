...
function hook(){
  ...
  const wrapped=async function(){
    ...
    if(id){
      const {data}=await db.from('matches').select('competition_id,status').eq('id',id).maybeSingle();
      if(data&&isFinal(data.status))await syncFromMatch(data);
    }
    return result;
  };
  wrapped.__afrnWrapped=true;
  window.saveMatch=wrapped;
}
setTimeout(hook,500);setTimeout(hook,1500);setTimeout(hook,3000);
const channel=db.channel('afrn-standings-sync').on('postgres_changes',{event:'*',schema:'public',table:'matches'},payload=>{if(payload.new&&payload.new.competition_id)syncFromMatch(payload.new);});
channel.subscribe();
})();
