...
/* Player registration authority: AFRN Admin/Super Admin only. */
(()=>{
'use strict';
const run=async()=>{
 ...
 try{
  ...
  const {data:p,error}=await db.from('profiles').select('role,club_id,full_name').eq('id',session.user.id).maybeSingle();if(error||!p)return;
  const role=String(p.role||'').toLowerCase();
  const isClub=['club_admin','club','club_account'].includes(role);
  if(isClub){
    ...
    if(banner){banner.textContent='🛡️ Usajili wa wachezaji unasimamiwa na AFRN Admin. Club Admin hawezi kuongeza wala kutoa AFRN Player ID. Wasilisha taarifa kwa AFRN kwa usajili rasmi.';banner.className='status warning';}
  }
 }catch(e){console.warn('AFRN player registration authority:',e.message||e)}
};
