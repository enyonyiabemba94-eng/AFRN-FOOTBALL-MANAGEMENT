...
const getRoot=()=>document.querySelector('#reportContainer .report')||document.querySelector('#transferPanel .table-wrap');
const transferRoot=()=>document.querySelector('#transferPanel');
...
async function exportPDF(){
 ...
 try{
  ...
  if(root.id==='transferPanel'){
   ...
   clone.style.background='#fff';clone.style.padding='20px';clone.style.display='block';clone.style.position='absolute';clone.style.left='-100000px';clone.style.top='0';clone.style.width='1100px';
   document.body.appendChild(clone);root=clone;
  }
  ...
  const pageW=210,pageH=297,margin=10,usableW=pageW-margin*2,pxPerMm=canvas.width/usableW,pagePx=Math.floor((pageH-margin*2)*pxPerMm);
  let y=0,page=0;
