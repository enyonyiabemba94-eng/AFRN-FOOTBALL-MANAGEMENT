...
function addCard(){
 ...
 section.innerHTML='<h3 style="margin:0 0 5px;color:#071a33">📊 Msimamo wa Ligi</h3><div id="dashStandingsSub" style="font-size:11px;color:#667085;margin-bottom:10px">Points → GD → GF → H2H → GA → Wins</div><div id="dashStandingsBody">Inapakia...</div><div id="dashAnalytics" style="margin-top:14px"></div>';
 const main=document.querySelector('main');if(main)main.appendChild(section);return section;
}
function h2h(a,b,matches){let ag=0,bg=0;for(const m of matches){if(String(m.home_team_id)===String(a)&&String(m.away_team_id)===String(b)){ag+=(m.home_score??0);bg+=(m.away_score??0)}else if(String(m.home_team_id)===String(b)&&String(m.away_team_id)===String(a)){ag+=(m.away_score??0);bg+=(m.home_score??0)}}return ag-bg}
function renderAnalytics(rows,finals,cm){
 const box=$('dashAnalytics');if(!box)return;
 const goals=finals.reduce((n,m)=>n+Number(m.home_score||0)+Number(m.away_score||0),0);
 const scorers=[...rows].sort((a,b)=>b.gf-a.gf);const leader=scorers[0];
 const avg=finals.length?(goals/finals.length).toFixed(2):'0.00';
...
async function render(){
 ...
 try{
  ...
  const finals=ms.filter(m=>m.competition_id===current.id);finals.forEach(m=>{if(m.home_team_id)teamIds.add(String(m.home_team_id));if(m.away_team_id)teamIds.add(String(m.away_team_id))});
  const rows=[...teamIds].map(id=>({id,group:(cts.find(t=>String(t.club_id)===id)||{}).group_name||'—',p:0,w:0,d:0,l:0,gf:0,ga:0}));const map=new Map(rows.map(r=>[r.id,r]));
  ...
  let myId=null;if(path.includes('club-dashboard')){const {data:{user}}=await db.auth.getUser();if(user){const r=await db.from('profiles').select('club_id').eq('id',user.id).maybeSingle();myId=r.data?.club_id?String(r.data.club_id):null}}
  ...
  renderAnalytics(rows,finals,cm);
